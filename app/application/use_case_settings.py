...
from app.infrastructure.persistence.models import UseCaseBinding
...
def resolve_foundry_binding(name: str) -> FoundryBinding:
    ...
    project_endpoint = os.getenv(
        f"{PROJECT_ENDPOINT_PREFIX}{normalized_name}", ""
    ).strip().rstrip("/")
    ...
    speech_endpoint = os.getenv(
        f"AZURE_SPEECH_ENDPOINT_{normalized_name}", ""
    ).strip().rstrip("/")
    ...
    if (
        speech_parsed.scheme != "https"
        or not speech_parsed.netloc.lower().endswith(".cognitiveservices.azure.com")
    ):
        raise ValueError(
...
def get_use_case_binding(use_case: str) -> UseCaseBinding | None:
    return get_repositories().use_case_settings.get_binding(use_case)


def resolve_use_case_binding(use_case: str) -> FoundryBinding | None:
    mapping = get_use_case_binding(use_case)
    return resolve_foundry_binding(mapping.binding) if mapping else None


def save_use_case_binding(use_case: str, binding: str) -> UseCaseBinding:
    ...
    get_repositories().use_case_settings.save_binding(mapping)
    return mapping
