...
async def stream_hosted_agent(message: str) -> AsyncIterator[dict[str, Any]]:
    settings = load_settings()
    if not settings.is_hosted_agent_configured:
        raise RuntimeError(
            "Hosted Agent is not configured. Set FOUNDRY_PROJECT_ENDPOINT and "
            "FOUNDRY_HOSTED_AGENT_NAME after deploying the agent."
        )
    if not settings.endpoint or not settings.hosted_agent_name:
