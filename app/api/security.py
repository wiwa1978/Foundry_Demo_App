...
from app.api.local_auth import AUTH_SESSION_COOKIE, decode_cookie, is_local_auth_configured
from app.core.config import env_csv, env_text
...
LOCAL_DEMO_SCOPE = identity.UserScope(tenant_id="local-demo", user_id="local-demo")
...
def require_privileged_user(connection: HTTPConnection) -> identity.UserScope:
...
def user_scope(connection: HTTPConnection) -> identity.UserScope:
    if auth_mode() is AuthMode.DISABLED:
        return LOCAL_DEMO_SCOPE
    user = authenticated_user(connection)
    if user is None:
    ...
    return identity.UserScope(tenant_id=tenant_id, user_id=user_id)
