import { Activity, Bot, Sparkles, Square, User, X } from "lucide-react";
...
export function AzureArchitectAgentWorkspace({
  ...
  onSubmit,
  onCancel,
  defaultAgentName = "azure-architect-prompt",
  emptyStateTitle = "Start your research",
  emptyStateDescription = "Ask the Azure Architect Agent to investigate a current topic using its configured web search tools.",
  questionPlaceholder = "Ask the Azure Architect Agent a question...",
  questionAriaLabel = "Azure Architect Agent question",
              ...
              <div className="grid gap-4">
                ...
                <div className="flex items-end gap-3">
                  ...
                  <div className="max-w-[min(52rem,86%)]">
                    <div className="mb-1 flex flex-wrap items-center gap-2 px-2 text-[11px] text-slate-500 dark:text-slate-400">
                      <span className="font-semibold text-slate-700 palette-accent-text">
                        {effectiveConfig.agentName || "Assistant"}
                      </span>
                    </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                      {submittedQuestion || question ||
                        "Ask something like 'What changed in Bing grounding this week?'"}
                    </p>
                  </div>
                  {isRunning ? <ThinkingIndicator /> : null}
                </div>
                <div className="mt-5 text-sm leading-7 text-slate-800 dark:text-slate-100">
                  {answer ? (
                    <MarkdownContent value={answer} />
                  ) : isRunning ? (
                    "Streaming answer..."
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs dark:border-[#606066] dark:bg-[#45454a]">
            ...
            <dl className="mt-3 grid gap-2">
              ...
              <ConfigRow
                ...
                value={
                  effectiveConfig.tracingEnabled ? "Enabled" : "Not configured"
                }
              />
            </dl>
          </div>
                  ...
                  >
                    ...
                    <div className="mt-2 grid gap-1 text-slate-500 dark:text-slate-400">
                      ...
                      span.output_tokens !== null ? (
                        <span>
                          Tokens: {span.input_tokens ?? 0} in /{" "}
                          {span.output_tokens ?? 0} out
                        </span>
                      ) : null}
                      {span.error_type ? (
