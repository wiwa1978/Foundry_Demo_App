        ...
        <header className="flex items-start justify-between gap-4 border-b px-5 py-4 dark:border-[#55555a]">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Amber rows differ. Green rules are enabled; muted rules are
              disabled.
            </p>
          </div>
          <Button
        ...
        <div className="overflow-auto p-5">
          <div className="grid min-w-[46rem] grid-cols-[12rem_minmax(0,1fr)_minmax(0,1fr)] gap-3">
            ...
            {filterNames.map((name, rowIndex) => {
              ...
              const showSection =
                rowIndex === 0 ||
                guardrailSection(filterNames[rowIndex - 1]) !== section;
              return (
                <Fragment key={name}>
                  {showSection ? (
                      ...
                      <span className="text-xs font-normal text-slate-400">
                        {
                          filterNames.filter(
                            (filterName) =>
                              guardrailSection(filterName) === section,
                          ).length
                        }{" "}
                        risk type(s)
                      </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        {enabledFilters.length ? (
                          <div className="mt-0.5 text-xs opacity-75">
                            Action:{" "}
                            {enabledFilters.every((filter) => filter.blocking)
                              ? "Block"
                              : "Annotate"}
                          </div>
                        ) : null}
