...
import { getAgentResearchTrace, streamAgentResearch } from "./api";
import type {
  AgentResearchCitation,
  AgentResearchRunConfig,
  AgentResearchStep,
  AgentResearchStreamEvent,
  AgentResearchTrace,
} from "./types";
...
const initialState: AgentResearchState = {
  question: "",
  answer: "",
  steps: [],
  citations: [],
...
export function useAgentResearchStream({
  fetchClient,
}: {
  fetchClient: FetchClient;
}) {
  const [state, setState] = useState<AgentResearchState>(initialState);
  ...
  async function loadTrace(
    ...
    try {
      for (let attempt = 0; attempt < tracePollAttempts; attempt += 1) {
        const trace = await getAgentResearchTrace({
          fetchClient,
          responseId,
          signal,
        });
