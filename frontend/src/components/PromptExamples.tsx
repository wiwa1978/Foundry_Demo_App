...
export function PromptExamples({
  ...
  function scroll(direction: -1 | 1) {
    scrollContainerRef.current?.scrollBy({
      left: direction * Math.max(280, scrollContainerRef.current.clientWidth * 0.75),
      behavior: "smooth",
    });
  }
  ...
  return (
    <section className="shrink-0 border-b bg-white px-4 py-3 dark:border-[#55555a] dark:bg-[#39393d]">
      <div className={cn("flex flex-wrap items-center justify-between gap-3", open && "mb-3")}>
        <button type="button" onClick={() => setOpen((current) => !current)} className="flex items-start gap-2 text-left">
        ...
        {open ? (
          <div className="flex items-center gap-1">
            <Button type="button" variant="outline" size="icon" className="h-8 w-8" onClick={() => scroll(-1)} disabled={!canScrollLeft} aria-label="Previous prompts">
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button type="button" variant="outline" size="icon" className="h-8 w-8" onClick={() => scroll(1)} disabled={!canScrollRight} aria-label="Next prompts">
      ...
      {open ? (
      <div ref={scrollContainerRef} className="prompt-carousel grid snap-x snap-mandatory grid-flow-col auto-cols-[minmax(15rem,20rem)] gap-2 overflow-x-auto overscroll-x-contain scroll-smooth">
