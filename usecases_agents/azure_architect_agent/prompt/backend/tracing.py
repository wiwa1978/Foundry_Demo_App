...
def get_azure_architect_agent_trace(response_id: str) -> AzureArchitectAgentTraceResponse:
    resource_id = load_settings().application_insights_resource_id
    if not resource_id:
        raise RuntimeError(
            "Azure Architect Agent tracing is not configured. Set "
    ...
    return AzureArchitectAgentTraceResponse(
...
def _spans_from_tables(tables: list[Any]) -> list[AzureArchitectAgentTraceSpan]:
