# Deployable hosted agent entry point.
#
# Unlike the local REPL examples, this module never calls
# input()/print() in a loop. Instead it starts an HTTP server implementing
# the OpenAI Responses protocol (ResponsesHostServer), which is what the
# Foundry hosting platform expects to find and route traffic to once
# deployed via create_version_from_code(...).
import logging
import os
...
def main() -> None:
    """Start the Responses protocol server used by Foundry hosting."""
    # FOUNDRY_PROJECT_ENDPOINT is injected automatically by the platform at
    # runtime once deployed. AZURE_AI_MODEL_DEPLOYMENT_NAME is set explicitly
    # via environment_variables in the deploy script below.
    ...
    # Hosted deployments cannot pause for an interactive approval prompt, so
    # this sample allows the Microsoft Learn MCP calls automatically.
    ...
    agent = Agent(
        ...
        # agent session), so the agent itself doesn't need to store it.
        default_options={"store": False},
    )

    # This server translates Foundry Responses API requests into Agent
    # Framework runs and manages the hosting protocol boundary.
