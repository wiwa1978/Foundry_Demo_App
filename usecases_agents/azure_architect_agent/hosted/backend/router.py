...
from usecases_agents.azure_architect_agent.hosted.backend.schemas import HostedAgentRequest
...
def hosted_agent_stream(
    ...
    async def encoded_events():
        async for event in stream_hosted_agent(request.message, request.agent_key):
