...
def test_trace_query_returns_bounded_safe_span_metadata():
    ...
    with (
        patch(
            "usecases_agents.azure_architect_agent.prompt.backend.tracing.load_settings",
            return_value=settings,
        ),
        patch("usecases_agents.azure_architect_agent.prompt.backend.tracing.get_azure_credential"),
        patch(
            "usecases_agents.azure_architect_agent.prompt.backend.tracing.LogsQueryClient",
            return_value=query_client,
        ),
    ):
        result = get_azure_architect_agent_trace("resp_test123")
