...
| `FOUNDRY_HOSTED_AGENT_NAME` | Optional hosted agent name. Defaults to `hosted-assistant`, matching `usecases_agents/azure_architect_agent/hosted/microsoft_agent_framework/azure.yaml`. |
...
| `AZURE_SPEECH_ENDPOINT` | Azure AI Speech custom-domain endpoint for Recorded Audio Transcription, Live translation, Text to Speech, and Text to Speech Avatar. |
...
| **Azure Architect Agent (Prompt Agent)** | Invokes the portal-built `azure-architect-prompt` agent by agent reference. Switchable from the "Agent implementation" dropdown alongside the hosted variant. [Browse the implementation](usecases_agents/azure_architect_agent/prompt/README.md). |
| **Azure Architect Agent (Hosted Agent)** | Invokes custom Microsoft Agent Framework code deployed and registered in Foundry Agent Service. Switchable from the "Agent implementation" dropdown alongside the prompt-agent variant. [Browse implementations and deployment guidance](usecases_agents/azure_architect_agent/hosted/README.md). |
| **Retail Shopping Assistant** | Consolidated SSE retail workspace using the repo2 Zava demo catalog, typed product events, and request-level cart round-tripping. [Browse the retail agent structure](usecases_agents/retail_agent/README.md). |
...
| **Text to Speech Avatar** | Submits a scripted Azure Speech batch avatar job, polls its status, and previews/downloads the generated MP4. [Browse the implementation](usecases_media/text_to_speech_avatar/README.md). |
...
The **Azure Speech Text to Speech** use case uses the Azure Speech SDK directly. It accepts text, selects an Azure Speech voice (including Dragon HD Latest voices), applies SSML emotion and prosody settings, and returns an MP3 for playback or download. Configure `AZURE_SPEECH_ENDPOINT` and either `AZURE_SPEECH_KEY` or Microsoft Entra access with the **Cognitive Services Speech User** role. The separate **Foundry GPT Audio** use case supports deployed `gpt-audio-mini` and `gpt-audio-1.5` models through the Foundry audio completions path; those models use their supported voice list and do not use Azure Speech SSML controls.

The **Text to Speech Avatar** use case uses the Azure Speech Batch Avatar REST API. The backend submits `PUT /avatar/batchsyntheses/{id}` with the script and avatar configuration, polls `GET /avatar/batchsyntheses/{id}`, and returns the signed result URL after the job succeeds. It supports standard video or photo avatars, configurable voices/backgrounds, and approved custom avatar or custom voice deployments. Configure `AZURE_SPEECH_ENDPOINT` and either `AZURE_SPEECH_KEY` or Microsoft Entra access with the **Cognitive Services Speech User** role. Batch avatar synthesis requires a supported paid Speech region.
