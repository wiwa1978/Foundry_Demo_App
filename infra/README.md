...
The deployed app requires Azure Container Apps built-in authentication by default. FastAPI uses `APP_AUTH_MODE=container_apps` and trusts only the encoded principal generated by the authentication proxy; local development uses `APP_AUTH_MODE=local` with the signed session cookie.

Set `adminPrincipals` in the selected `.bicepparam` file to a comma-separated administrator allowlist. Prefer immutable Microsoft Entra object IDs in production; normalized email addresses are supported for local ergonomics. The safe default is empty, which denies all privileged operations while authentication is enabled. Both deployment templates provision this value as `ADMIN_PRINCIPALS` on the Container App.
